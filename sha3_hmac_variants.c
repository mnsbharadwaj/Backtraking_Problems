...
typedef struct {
    ...
    int variant;
    size_t hash_size;#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <libkeccak.h>
...
int hmac_sha3_init(hmac_sha3_context_t* ctx, int variant, const void* key, size_t key_len) {
    ...
    // Allocate buffers
    ctx->padded_key = calloc(ctx->block_size, 1);
    ctx->inner_hash = malloc(ctx->inner_ctx.hash_size);
    
    if (!ctx->padded_key || !ctx->inner_hash) {
    ...
    if (key_len > ctx->block_size) {
        ...
        // Rest is already zeros from calloc
    }
    
    // Create ipad and start inner hash
    unsigned char* ipad = malloc(ctx->block_size);
    if (!ipad) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        free(ctx->padded_key);
        free(ctx->inner_hash);
...
int hmac_sha3_finalize(hmac_sha3_context_t* ctx, unsigned char* output) {
    ...
    // Create opad and start outer hash
    unsigned char* opad = malloc(ctx->block_size);
    if (!opad) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        return -1;
    }
    ...
    // Outer hash: SHA3(opad || inner_hash)
    if (sha3_process(&ctx->outer_ctx, opad, ctx->block_size) < 0) {
        free(opad);
        return -1;
    }
...
int hmac_sha3_oneshot(const char* key, size_t key_len, const char* message, size_t message_len, int variant) {
    ...
    output = malloc(ctx.inner_ctx.hash_size);
    if (!output) {
        fprintf(stderr, "Error: Memory allocation failed\n");
        hmac_sha3_cleanup(&ctx);
        return -1;
...
void print_usage(const char* program_name) {
    printf("SHA-3 and HMAC-SHA3 Calculator - Init/Process/Finalize Architecture\n");
    printf("Usage: %s [OPTIONS] [ARGS]\n\n", program_name);
